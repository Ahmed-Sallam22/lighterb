...
const GLPeriodsPage = () => {
	...
	useEffect(() => {
		document.title = `${t("glPeriods.title")} - LightERP`;
		return () => {
	...
	const columns = useMemo(
		() => [
			{
				header: t("glPeriods.table.periodNumber"),
			...
			{
				header: t("glPeriods.table.endDate"),
				accessor: "end_date",
				render: value => (
					<span className="text-gray-700">
						{value ? new Date(value).toLocaleDateString(i18n.language) : "-"}
			...
			{
				header: t("glPeriods.table.state"),
				accessor: "state",
				width: "120px",
				render: value => {
					const stateColors = {
					...
					return (
						...
						>
							{t(`glPeriods.states.${value}`) || value}
						</span>
					);
				},
			},
	...
	const handleView = async row => {
		try {
			await dispatch(fetchGLPeriod(row.id)).unwrap();
			setIsViewModalOpen(true);
		} catch {
			toast.error(t("glPeriods.messages.fetchError"));
		}
	};
	...
	const handleConfirmAction = async () => {
		...
		try {
			if (confirmAction === "open") {
				await dispatch(openGLPeriod(selectedPeriodForAction.id)).unwrap();
				toast.success(t("glPeriods.messages.openSuccess"));
			} else if (confirmAction === "close") {
				await dispatch(closeGLPeriod(selectedPeriodForAction.id)).unwrap();
				toast.success(t("glPeriods.messages.closeSuccess"));
			}
		} catch (error) {
			const errorMessage = typeof error === "string" ? error : t("glPeriods.messages.actionError");
	...
	const customActions = [
		{
			...
			title: t("glPeriods.actions.open"),
		...
		},
		{
			...
			title: t("glPeriods.actions.close"),
	...
	return (
		<div className="min-h-screen bg-gray-50">
			...
			<PageHeader
				title={t("glPeriods.title")}
				subtitle={t("glPeriods.subtitle")}
				...
				<div className="bg-white rounded-xl shadow-sm p-4 mb-6">
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
						...
						<div>
							...
							>
								<option value="">{t("glPeriods.filters.allStates")}</option>
								<option value="open">{t("glPeriods.states.open")}</option>
								<option value="closed">{t("glPeriods.states.closed")}</option>
								<option value="hold">{t("glPeriods.states.hold")}</option>
							</select>
						</div>
						...
						<div>
							<label className="block text-sm font-medium text-gray-700 mb-1">
								{t("glPeriods.filters.fiscalYear")}
							</label>
							<select
								...
								className="w-full h-10 px-3 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-[#48C1F0] focus:border-[#48C1F0] cursor-pointer"
							>
								<option value="">{t("glPeriods.filters.allYears")}</option>
				...
				{loading ? (
					...
					<>
						<Table
							...
							emptyMessage={t("glPeriods.emptyMessage")}
						/>
			...
			<SlideUpModal
				...
				title={t("glPeriods.modals.viewTitle")}
				maxWidth="600px"
			>
				{selectedPeriod && (
					<div className="space-y-4 p-4">
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								<label className="block text-sm font-medium text-gray-500">
									{t("glPeriods.details.fiscalYear")}
								</label>
							...
							<div>
								...
								>
									{t(`glPeriods.states.${selectedPeriod.state}`) || selectedPeriod.state}
								</span>
							</div>
							<div>
								<label className="block text-sm font-medium text-gray-500">
									{t("glPeriods.details.startDate")}
								</label>
			...
			<ConfirmModal
				...
				title={confirmAction === "open" ? t("glPeriods.modals.openTitle") : t("glPeriods.modals.closeTitle")}
				message={
					confirmAction === "open"
						? t("glPeriods.modals.openMessage", {
								period: selectedPeriodForAction?.period_name || "",
						  })
						: t("glPeriods.modals.closeMessage", {
								period: selectedPeriodForAction?.period_name || "",
						  })
				}
				confirmText={confirmAction === "open" ? t("glPeriods.actions.open") : t("glPeriods.actions.close")}
