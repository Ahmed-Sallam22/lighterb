...
const ARReceiptsPage = () => {
	...
	const customerOptions = [
		{ value: "", label: t("payments.filters.allCustomers") },
	...
	const handleSubmitForApproval = async payment => {
		try {
			await dispatch(submitARPaymentForApproval(payment.id)).unwrap();
	...
	const handlePostToGL = async payment => {
		try {
			await dispatch(postARPaymentToGL(payment.id)).unwrap();
				...
				{loading ? (
					...
					<>
						<Table
							...
							emptyMessage={t("payments.ar.emptyMessage")}
						/>
