...
const ARPeriodsPage = () => {
	...
	const columns = useMemo(
		() => [
			{
				header: t("arPeriods.table.periodNumber"),
			...
			{
				header: t("arPeriods.table.endDate"),
				accessor: "end_date",
				render: value => (
					<span className="text-gray-700">
						{value ? new Date(value).toLocaleDateString(i18n.language) : "-"}
			...
			{
				header: t("arPeriods.table.state"),
				accessor: "state",
				width: "120px",
				render: value => {
					const stateColors = {
					...
					return (
						...
						>
							{t(`arPeriods.states.${value}`) || value}
						</span>
					);
				},
			},
	...
	const handleView = async row => {
		try {
			await dispatch(fetchARPeriod(row.id)).unwrap();
			setIsViewModalOpen(true);
		} catch {
			toast.error(t("arPeriods.messages.fetchError"));
		}
	};
	...
	const handleConfirmAction = async () => {
		...
		try {
			if (confirmAction === "open") {
				await dispatch(openARPeriod(selectedPeriodForAction.id)).unwrap();
				toast.success(t("arPeriods.messages.openSuccess"));
			} else if (confirmAction === "close") {
				await dispatch(closeARPeriod(selectedPeriodForAction.id)).unwrap();
				toast.success(t("arPeriods.messages.closeSuccess"));
			}
		} catch (error) {
			const errorMessage = typeof error === "string" ? error : t("arPeriods.messages.actionError");
	...
	const customActions = [
		{
			...
			title: t("arPeriods.actions.open"),
		...
		},
		{
			...
			title: t("arPeriods.actions.close"),
	...
	return (
		<div className="min-h-screen bg-gray-50">
			...
			<PageHeader
				title={t("arPeriods.title")}
				subtitle={t("arPeriods.subtitle")}
				...
				<div className="bg-white rounded-xl shadow-sm p-4 mb-6">
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
						...
						<div>
							...
							>
								<option value="">{t("arPeriods.filters.allStates")}</option>
								<option value="open">{t("arPeriods.states.open")}</option>
								<option value="closed">{t("arPeriods.states.closed")}</option>
								<option value="hold">{t("arPeriods.states.hold")}</option>
							</select>
						</div>
						...
						<div>
							<label className="block text-sm font-medium text-gray-700 mb-1">
								{t("arPeriods.filters.fiscalYear")}
							</label>
							<select
								...
								className="w-full h-10 px-3 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-[#48C1F0] focus:border-[#48C1F0] cursor-pointer"
							>
								<option value="">{t("arPeriods.filters.allYears")}</option>
				...
				{loading ? (
					...
					<>
						<Table
							...
							emptyMessage={t("arPeriods.emptyMessage")}
						/>
			...
			<SlideUpModal
				...
				title={t("arPeriods.modals.viewTitle")}
				maxWidth="600px"
			>
				{selectedPeriod && (
					<div className="space-y-4 p-4">
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								<label className="block text-sm font-medium text-gray-500">
									{t("arPeriods.details.fiscalYear")}
								</label>
							...
							<div>
								...
								>
									{t(`arPeriods.states.${selectedPeriod.state}`) || selectedPeriod.state}
								</span>
							</div>
							<div>
								<label className="block text-sm font-medium text-gray-500">
									{t("arPeriods.details.startDate")}
								</label>
			...
			<ConfirmModal
				...
				title={confirmAction === "open" ? t("arPeriods.modals.openTitle") : t("arPeriods.modals.closeTitle")}
				message={
					confirmAction === "open"
						? t("arPeriods.modals.openMessage", {
								period: selectedPeriodForAction?.period_name || "",
						  })
						: t("arPeriods.modals.closeMessage", {
								period: selectedPeriodForAction?.period_name || "",
						  })
				}
				confirmText={confirmAction === "open" ? t("arPeriods.actions.open") : t("arPeriods.actions.close")}
