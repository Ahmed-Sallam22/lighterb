...
const InvoiceApprovalsPage = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast.success('Invoice approval deleted successfully');
      setIsDeleteModalOpen(false);
      setDeleteId(null);
    } catch (error) {
      toast.error(error || 'Failed to delete invoice approval');
    }
  };
  ...
  const confirmApprove = async () => {
    try {
      ...
      toast.error(error || 'Failed to approve invoice');
    }
  };
  ...
  const confirmReject = async () => {
    try {
      ...
      toast.error(error || 'Failed to reject invoice');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingApproval) {
        ...
        toast.success('Invoice approval created successfully');
      }
      ...
      toast.error(error || 'Failed to save invoice approval');
    }
  };
  ...
  const mappedApprovals = approvals.map((approval) => ({
    id: approval.id,
    type: approval.invoice_type === 'AP' ? 'AP Invoice' : 'AR Invoice',
    invoiceNumber: `INV-${approval.invoice_id}`,
    submittedBy: approval.submitted_by,
    approver: approval.approver || 'N/A',
    status: approval.status === 'PENDING_APPROVAL' ? 'Pending' : approval.status === 'APPROVED' ? 'Approved' : 'Rejected',
    ...
    },
    {
      ...
      icon: (
        <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M28 21.333V27.9997L32.1667 32.1663" stroke="#FFC043" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
    },
    {
      ...
      icon: (
        <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect width="56" height="56" rx="28" fill="#F5F7FA"/>
          <path d="M27.9998 43.2379C36.4156 43.2379 43.2379 36.4156 43.2379 27.9998C43.2379 19.584 36.4156 12.7617 27.9998 12.7617C19.584 12.7617 12.7617 19.584 12.7617 27.9998C12.7617 36.4156 19.584 43.2379 27.9998 43.2379Z" stroke="#D44333" strokeWidth="1.90476" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M22.2852 22.2861L33.7137 33.7147M33.7137 22.2861L22.2852 33.7147" stroke="#D44333" strokeWidth="1.90476" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
    },
  ];
    ...
    <div className="min-h-screen bg-gray-50">
      <PageHeader
        ...
        icon={
          <svg width="29" height="35" viewBox="0 0 29 35" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" clipRule="evenodd" d="M5.83125 0.0830994C5.91135 0.0830994 5.99265 0.0830994 6.07509 0.0830994H21.9251C22.0076 0.0830994 22.0888 0.0830994 22.1689 0.0830994C23.8635 0.0822937 25.0297 0.0818024 26.0119 0.423639C27.8669 1.06904 29.307 2.56176 29.9245 4.44232C30.2512 5.43754 30.2508 6.62062 30.2502 8.37732C30.2501 8.45589 30.2501 8.53557 30.2501 8.61659V31.9568C30.2501 34.3989 27.3718 35.8527 25.4761 34.118C25.3436 33.9968 25.1566 33.9968 25.0241 34.118L24.219 34.8548C22.672 36.2704 20.3282 36.2704 18.7813 34.8548C18.1896 34.3134 17.3106 34.3134 16.719 34.8548C15.172 36.2704 12.8282 36.2704 11.2813 34.8548C10.6896 34.3134 9.81057 34.3134 9.21895 34.8548C7.67199 36.2704 5.32819 36.2704 3.78124 34.8548L2.97609 34.118C2.84358 33.9968 2.65659 33.9968 2.52408 34.118C0.628358 35.8527 -2.24988 34.3989 -2.24988 31.9568V8.61659C-2.24988 8.53557 -2.24989 8.45589 -2.24989 8.37732C-2.25057 6.62062 -2.25098 5.43754 -2.07568 4.44232C-1.45819 2.56176 -0.0180215 1.06904 1.83689 0.423639C2.81905 0.0818024 3.98526 0.0822937 5.67988 0.0830994H5.83125Z" fill="#28819C"/>
          </svg>
        }
      />
      ...
      <div className="w-[95%] mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Invoice Approvals</h2>
      ...
      <SlideUpModal
        ...
        title={editingApproval ? 'Edit Invoice Approval' : 'Create Invoice Approval'}
      >
        <div className="space-y-4">
          <FloatingLabelSelect
            ...
            options={[
              { value: 'AP', label: 'AP Invoice' },
              { value: 'AR', label: 'AR Invoice' },
            ]}
            required
          />
          ...
          <FloatingLabelSelect
            ...
            options={availableInvoices.map((invoice) => ({
              ...
              label: `#${invoice.id} - ${invoice.customer?.name || invoice.supplier?.name || invoice.vendor?.name || 'N/A'} - ${invoice.total || invoice.amount || 0}`,
            }))}
            ...
            >
              {loading ? (
                <span className="flex items-center justify-center">
                  ...
                  {editingApproval ? 'Updating...' : 'Creating...'}
                </span>
              ) : (
      ...
      <ConfirmModal
        ...
        title="Delete Invoice Approval"
        message="Are you sure you want to delete this invoice approval? This action cannot be undone."
        confirmText="Delete"
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-600">
            Are you sure you want to approve this invoice? You can add optional comments below.
          </p>
