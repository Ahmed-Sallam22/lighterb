...
const QuickActionDetail = () => {
  ...
  const action = useMemo(
    () => getQuickActionById(actionId) || QUICK_ACTIONS[0],
    [actionId]
  );

  useEffect(() => {
    const pageTitle = action?.label
      ? `Light ERP | ${action.label}`
      : 'Light ERP | Quick Actions';
        ...
        return (
          <div className="max-w-4xl mx-auto mt-5 pb-10">
            <div className="rounded-4xl bg-white shadow-xl border border-white/60 p-12 text-center">
              <p className="text-xl text-[#28819C] font-semibold mb-4">
                {action?.label}
              </p>
              <p className="text-[#7A9098]">
