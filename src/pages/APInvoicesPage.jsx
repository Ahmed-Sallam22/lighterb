...
const APInvoicesPage = () => {
  ...
  const [actionType, setActionType] = useState(null); // 'delete', 'reverse', 'post', 'submit'

  const title = 'AP Invoices';
  const subtitle = 'Accounts Payable Invoices - Supplier Bills';
  const icon = <APInvoiceIcon />;
  const buttonText = 'New AP Invoice';
  const quickActionPath = '/quick-actions/create-ap-invoice';
  ...
  const sampleData = invoices.map((invoice) => ({
    ...
    approvalStatus:
      ...
      invoice.approval_status === 'REJECTED' ? 'Rejected' : 'Draft',
    rawData: invoice,
  }));
  ...
  const handleSubmitForApproval = async (row) => {
    const invoice = row.rawData || row;
    try {
      await dispatch(submitAPInvoiceForApproval(invoice.id)).unwrap();
  ...
  const handleThreeWayMatch = async (row) => {
    const invoice = row.rawData || row;
    try {
      await dispatch(threeWayMatchAPInvoice(invoice.id)).unwrap();
  ...
  const handleConfirmAction = async () => {
    try {
      if (actionType === 'delete') {
        await dispatch(deleteAPInvoice(selectedInvoice.id)).unwrap();
        toast.success('Invoice deleted successfully');
      } else if (actionType === 'reverse') {
        await dispatch(reverseAPInvoice(selectedInvoice.id)).unwrap();
        toast.success('Invoice reversed successfully');
      } else if (actionType === 'post') {
        await dispatch(postAPInvoiceToGL(selectedInvoice.id)).unwrap();
