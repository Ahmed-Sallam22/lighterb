...
<path fillRule="evenodd" clipRule="evenodd" d="M2.16939 -3.49691e-07L10.8474 8L2.16939 16L-0.000120827 14L6.5084 8L-0.000120258 2L2.16939 -3.49691e-07Z" fill="#48C1F0"/>
</svg>
        ...
        {/* Cards Section */}
        <div 
          className={`grid grid-cols-1 md:grid-cols-2 ${cards.length > 2 ? 'lg:grid-cols-4' : 'lg:grid-cols-2'} gap-6 transition-all duration-500 ${
          ...
          {cards.map((card, index) => (
            <div
              ...
              className={`rounded-xl border border-white/10 bg-[#28819C]/30 py-6 px-3 shadow-xl transition-all duration-500 transform hover:-translate-y-2 hover:bg-white/20 hover:shadow-[0_10px_30px_rgba(72,193,240,0.3)] animate-fadeInUp ${
                (activeRoute === 'Journal' || (activeRoute === 'Dashboard' && (card.title === 'Setup' || card.title === 'Journal Entries' || card.title === 'Journal Lines' || card.title === 'Currencies' || card.title === 'Exchange Rates' || card.title === 'Tax Rates' || card.title === 'Invoice Approvals'))) ? 'cursor-pointer' : 'cursor-pointer'
              }`}
              style={{
