...
const FloatingLabelInput = ({ 
  ...
  return (
    <div className={`relative ${className}`}>
      <div
        className={`
          relative rounded-[20px] p-[2px] transition-all duration-300
      ...
      >
        ...
        <input
          ...
          className={`
            w-full bg-transparent text-sm rounded-[18px] border-none
            px-5 py-3  text-[#031b28]
            ${isDateField
            ...
            ${(isDateField || type === 'password') ? 'pr-12' : ''}
            ${isDateField ? '[&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:w-full [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:cursor-pointer' : ''}
          `}
        />
        ...
        <label
          ...
          className={`
            ...
            ${isFloating
              ? '-top-3 text-sm px-2 rounded-full bg-[white] text-[#28819C]'
        ...
        {type === 'date' && (
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer hover:scale-110 transition-transform duration-200"
            disabled={disabled}
          >
            <CalendarIcon />
          </button>
        ...
        {/* Error Message */}
        {hasError && (
          <p id={errorId} className="absolute -bottom-6 left-1 text-sm text-red-400 flex items-center gap-1">
